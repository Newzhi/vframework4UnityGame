// ABundleLoader.cs — ④ 加载器（Layer4_Loader）
// 用途：对外薄 API（Initialize、LoadHandle、LoadAsset、Release），委托 ③ 抽象资源层处理包与引用计数。
...
namespace vFramework.BaseLayer.AssetLayer.ABundleLayer
{
    ...
    public class ABundleLoader : IABundleLoader
    {
        ...
        public IAssetHandle LoadHandle<T>(string location) where T : UnityEngine.Object
        {
            ...
            {
                var editorAsset = LoadFromEditor<T>(entry);
                return new ABundleAssetHandle(location, editorAsset, null, null);
            }
#endif
            ...
            var asset = bundle == null ? null : bundle.LoadAsset<T>(entry.AssetName);
            if (asset == null)
            {
            ...
            return new ABundleAssetHandle(location, asset, ticket, _resources.ReleaseTicket);
        }
        ...
        public void LoadHandleAsync<T>(string location, Action<IAssetHandle> onComplete) where T : UnityEngine.Object
        {
            onComplete?.Invoke(LoadHandle<T>(location));
        }
