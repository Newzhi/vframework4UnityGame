...
业务代码
    ...
    ▼
BundleResLoader          ← 单例 + 首次 Load 懒 Init；查 Catalogue、缓存 AbstractResource
    ▼
...
| `LoadAsync` / 回调 / 预加载包 | **LoadAsync 为设计基线默认 API**（未实现）；回调、预加载 TODO | 📋 |
...
- 设计基线要求 **异步为默认 API**，对应 `LoadAsync`（待实现）；当前同步 `Load` 为已实现入口。
...
**下一步（建议顺序）**：异步 Load 默认 API → AssetRouter → **CDN 下载（IRemoteBundleProvider）**。
...
| 2 异步加载（**设计基线默认 API**） | ❌ `LoadAsync` 占位 |
