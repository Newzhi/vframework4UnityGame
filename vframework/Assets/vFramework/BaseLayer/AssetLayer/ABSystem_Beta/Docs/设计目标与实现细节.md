...
| [业务API调用指南.md](./业务API调用指南.md) | **Load/Release 抄用范式**（与本节场景配套） |
...
> **入口**：`BundleResLoader.Instance` + `IAssetHandle`；卸载见 [业务API调用指南.md](./业务API调用指南.md)。  
> **架构图**：[ResLoader/说明.md](../ResLoader/说明.md) § 加载侧架构图。
...
详见 [ResLoader/说明.md](../ResLoader/说明.md)、[业务API与CDN规划.md](./业务API与CDN规划.md)。
...
| 文档 | 用途 |
|------|------|
| [业务API调用指南.md](./业务API调用指南.md) | 代码范式、Unload 组合 |
| [主路线.md](./主路线.md) | PreLoad / 真异步 / CDN 排期 |
...
├── ResLoader/               # Business / Bundle / Catalogue / Router + 说明.md
