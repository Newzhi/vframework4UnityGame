...
> **本阶段**：加载侧已打通 Catalogue 读、同步 `Load(简路径)` 与依赖预加载；**LoadUniTaskAsync（UniTask）已接入基础版**，路由器与 CDN 下载待做。
...
| 2 | 异步加载（设计基线 **默认 API**） | ✅ | `LoadUniTaskAsync(loadPath)` 已接入 UniTask（当前内部仍复用同步 Load） |
| 3 | 加载 + 回调 | ✅ | `LoadUniTaskWithCallback(path, onComplete, onFailed, useUniTask)` 已实现 |
| 4 | 预加载资源包 | ❌ | `PreLoad` 占位 |
...
**阶段判断（加载）**：同步 + 清单驱动 + 依赖预加载已通；**异步入口已提供**，CDN 下载与路由器为下一阶段。
...
| **4. 加载器 + 路由器** | ✅ | **约 55%** | 同步 `Load(简路径)`、依赖预加载 ✅；`LoadUniTaskAsync(UniTask)` ✅；预加载/CDN/路由器 ❌ |

**阶段判断**：已完成 **「打包 → 清单 → 同步 Load(简路径) + 依赖预加载」** 主链路；距离 **LoadUniTaskAsync（默认 API）**、CDN 运行时下载、AssetRouter 仍有差距。
...
| `BundleResLoader` | 同步 `Load<T>(loadPath)`；辅助 `LoadByBundle` / `LoadByAssetPath`；`LoadUniTaskAsync(loadPath)`（UniTask）+ 回调加载 |
| `AssetRouter` / 预加载 / CDN 下载 / 竞态合并 | 未做（见 [业务API与CDN规划.md](./业务API与CDN规划.md)） |

设计基线 §1 同步路径：`Load("Atlas/Role/资源名")` 已实现；§2 **默认 API** 的 `LoadUniTaskAsync(loadPath)` 已接入 UniTask（内部仍复用同步 Load），并提供 `LoadUniTaskWithCallback(...)` 系列回调入口。
