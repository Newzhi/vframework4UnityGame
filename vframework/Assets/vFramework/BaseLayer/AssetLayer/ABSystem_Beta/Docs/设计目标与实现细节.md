...
> **本阶段**：打包侧 MVP 已通；加载侧已打通 Catalogue 读、LoadByPath 与依赖预加载；异步/路由器待做。
...
| 2 | 异步加载（默认 API） | ❌ | `LoadAsync` 占位，计划 UniTask |
...
| **4. 加载器 + 路由器** | ✅ | **约 45%** | 同步 `LoadByPath`、依赖预加载 ✅；异步/预加载/CDN 下载/路由器 ❌ |

**阶段判断**：已完成 **「打包规则 → 打 AB → 写 JSON 清单（含 bundles）→ 运行时 LoadByPath + 依赖预加载」** 主链路；距离设计目标中的 **CDN 运行时下载、异步默认 API、AssetRouter** 仍有差距。
...
| `BundleResLoader` | `Load<T>(bundleName, assetName)`、`LoadByPath<T>(assetPath)`、`Init` 加载 Catalogue |
| `AssetRouter` / 异步 Load / 预加载 / CDN 下载 / 竞态合并 | 未做（见 [业务API与CDN规划.md](./业务API与CDN规划.md)） |

目标 API `Load<GameObject>("e.Path")` 的同步路径已通过 **LoadByPath** 实现；路由器与异步 API 待后续阶段。
...
| 字段 | 默认 | 用途 |
|------|------|------|
| `deviceOutputPath` | `Assets/StreamingAssets` | 真机模式/首包 |
| `cdnOutputPath` | `Bundles/CDN` | CDN联网 |
...
| DeviceDebug | 真机模式/首包 | `deviceOutputPath` | 仍写 JSON |
| CdnHotUpdate | CDN联网 | `cdnOutputPath` | 仍写 JSON |
...
4. **Play 加载**：`BundleResLoader.Init(bundleRoot)` → `LoadByPath` 或 `Load(bundle, asset)`；跨包资源验证依赖预加载（L-024）  
...
| 产物 | 路径 |
|------|------|
| AB 包（真机模式/首包） | `{BuildSetting.deviceOutputPath}`，默认 `Assets/StreamingAssets/*.bundle` |
...
（加载侧：用 `ABLoadSmokeTest` 或 PlayMode 执行 L-024 / L-033。）
