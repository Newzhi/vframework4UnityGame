...
| 资源清单 | `Assets/vFramework/BaseLayer/AssetLayer/ABSystem_Beta/BundleRuleConfig/Catalogue/AssetCatalog.json` |
...
| **ABSystem_Beta（本项目）** | 规划中（Catalogue + Editor 直读） | 当前：`StreamingAssets` | 当前：`StreamingAssets` | 规划中（CDN 模式） |
...
- **ABSystem_Beta 当前阶段**：以 **真机构建 + Catalogue 双份输出** 为测试标准；开发期可暂全进 `StreamingAssets`，CDN 模式与首包拆分后续实现。