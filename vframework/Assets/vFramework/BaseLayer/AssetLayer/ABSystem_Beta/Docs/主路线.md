...
| **B-Pool-1** | 对象池 **业务范式**落地（句柄长期持有、Return 不 Release） | 文档 + 业务侧可抄 [业务API调用指南 §7.7](./业务API调用指南.md) |
...
**原则**：对象池 **不新增** Load/Release API；在现有 `IAssetHandle` 上约定用法（见 [业务API调用指南 §6/§7.7](./业务API调用指南.md)）。
...
| # | 项 | 完成标准 |
|---|-----|----------|
| 1 | 对象池范式审查 | [业务API调用指南 §7.7](./业务API调用指南.md) 与 Case 一致 |
