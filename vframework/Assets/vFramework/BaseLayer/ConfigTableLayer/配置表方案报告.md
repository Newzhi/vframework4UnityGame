...
> Excel 源表始终不在 Unity 目录；Unity 保留 **HotUpdateScripts/MetaConfigs 代码** 与 **AssetBundle/ConfigTables 下 bytes**（Default 规则 → `configtables.bundle`）。详见 [配表工具生成契约.md](./配表工具生成契约.md)。
...
| 目录 | 内容 | 打包归属 | 是否可热更 |
|---|---|---|---|
| `HotUpdateScripts/MetaConfigs/` | Meta / Table / GameConfigTables | HotUpdateScripts 程序集 | 表结构可热更 |
| `AssetBundle/ConfigTables/` | `.bytes` 源资源 | `configtables.bundle` | 数据可 CDN 热更 |

运行时 XTBL 读取器：**`BaseLayer.GameUtils.XtblBinaryReader`**（手写固定，工具不再生成 `BinaryTypeReader`）。

### 4.2 AB 打包（vFramework AssetBundle Packer）

`Assets/AssetBundle/ConfigTables/` 在 **Default** 规则下自动打成 **`configtables.bundle`**。运行时 loadPath：`ConfigTables/{表名}`。

框架接入：**`ConfigTableModule`**（Priority 120）预加载 bytes → `GameConfigTables.Initialize`；生成类内 **`RegisterToFramework`** 注册解析回调。
...
- [x] `ConfigTableModule` + `ConfigTableBytesLoader`：AB 加载 + 两阶段初始化
- [ ] AssetBundle Packer 构建验证：`configtables.bundle` 入 catalogue
