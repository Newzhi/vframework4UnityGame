...
| `BinaryTypeReader`（共享运行时） | `BinaryTypeReader.cs` | 与 `BinaryTypeCodec` 对称，解析 bytes |
...
```json
{
  ...
  "codeOutput": "D:/MyGame/Assets/Config/Generated",
  "tablesOutput": "D:/MyGame/Assets/Config/Generated/Tables",
  "dataOutput": "D:/MyGame/Assets/ABundles/Configs",
...
> Excel 源表始终不在 Unity 目录；Unity 只保留 **Generated 代码** 与 **ABundles 下的 bytes 源资源**（由项目自定义打包管线打成 AB）。
...
Assets/
  Config/
    Generated/                    ← codeOutput + tablesOutput
      Metas/
        HeroMeta.cs
        ItemMeta.cs
      Tables/
      ...
      Runtime/
        BinaryTypeReader.cs       ← 手写一份，类型不变则不改
  ABundles/
    Configs/                      ← dataOutput，非热更表（随包）
      Hero.bytes
      Item.bytes
      ...
    ConfigsHot/                   ← 可选，热更表（CDN 替换）
      Hero.bytes                  ← 同名表可覆盖随包版本
      ...
```

| 目录 | 内容 | 打包归属 | 是否可热更 |
|---|---|---|---|
| `Config/Generated/` | C# 代码（Meta / Table / Global） | 编译进 DLL | 否（随版本发） |
| `ABundles/Configs/` | 配置表 `.bytes` 源资源 | 打成随包 AB | 否 |
| `ABundles/ConfigsHot/` | 配置表 `.bytes` 源资源（可选） | 打成热更 AB | **是** |

**目录数量约定：**

- **常见情况（单目录）：** 所有表 bytes 导出到 `ABundles/Configs/`，打一个 Config AB，随安装包发布。
- **需要热更时（双目录）：** 将可热更的表放到 `ABundles/ConfigsHot/`（或策划在导表配置中指定输出子目录），与 `Configs/` 分开打包；运行时先加载随包 AB，再加载热更 AB，**同名 `*.bytes` 以热更为准**。

> 不按「每张表一个 AB」拆分。表数量再多，通常也只占 **1～2 个 AB 目录**（非热更 / 热更），每张表仍是目录内的一个 `TextAsset`（`Hero.bytes` 等）。

### 4.2 AB 打包（项目自定义管线）

本项目 **不在 ExcelTool 内实现 AB 打包**；由 Unity 工程已有的 **自定义打包工具** 扫描 `ABundles/` 下各子文件夹，按文件夹选择打包策略与输出方式。

```
ABundles/
  Configs/       → 打包策略 A（随包 / Local）
  ConfigsHot/    → 打包策略 B（热更 / CDN）
  ...            → 其他资源类型，与配置表无关
```
...
| 职责 | 说明 |
|---|---|
| 按文件夹选策略 | `ABundles/Configs` 与 `ABundles/ConfigsHot` 可走不同输出路径、压缩、加密、平台分包规则 |
| 产出 AB 文件 | 例如 `configs`、`configs_hot` 等 bundle 名，由项目规范决定 |
| 热更清单 | 热更 AB 的版本号 / MD5 / CDN 路径由资源热更系统维护，配置表只消费加载结果 |

导表工具职责边界：

- 只负责把 `*.bytes` 写到 `dataOutput` 指向的 `ABundles/...` 目录
- 不关心 AB 如何打、打几个、热更 URL 是什么
...
public class ConfigBootstrap : MonoBehaviour
{
    async void Awake()
    {
        // ① 预加载 AB（随包 + 可选热更，由项目 AB 系统按 ABundles 目录规则加载）
        var cache = new Dictionary<string, byte[]>();

        var abLocal = await AbManager.LoadAsync("configs");           // ← ABundles/Configs
        MergeBytes(cache, abLocal);

        var abHot = await AbManager.TryLoadHotAsync("configs_hot");   // ← ABundles/ConfigsHot，可选
        if (abHot != null)
            MergeBytes(cache, abHot);  // 同名覆盖

        // ② 解析
        GameConfigTables.Initialize(cache);

        DontDestroyOnLoad(gameObject);
    }

    static void MergeBytes(Dictionary<string, byte[]> cache, AssetBundle ab)
    {
        foreach (var asset in ab.LoadAllAssets<TextAsset>())
            cache[asset.name] = asset.bytes;  // TextAsset 名即 Hero.bytes（无扩展名时为 Hero）
    }
}
```

> `Awake` 里只走一遍初始化链路。表增删由导表工具更新 `GameConfigTables`；预加载侧可通过 `LoadAllAssets<TextAsset>()` 扫齐 AB 内全部表，无需手写表名列表。AB bundle 名（`configs` / `configs_hot`）与项目自定义打包输出保持一致即可。
...
- [ ] `BinaryTypeReader.cs`（共享运行时读取器）
- [ ] `TableCodeGenerator`（`Load` + `GetByMainID`）
- [ ] `GlobalTablesGenerator`（`GameConfigTables`，随表列表自动更新）
- [ ] `ExportPipeline` 收集全表后生成全局类
- [ ] `export-config.json` 增加 `tablesOutput`
- [ ] 文档与 gitignore 同步

### 9.3 待实现（Unity 侧，手写）

- [ ] `ConfigBootstrap`：AB 加载 + 两阶段初始化
- [ ] 与现有自定义打包对齐：`ABundles/Configs`、`ABundles/ConfigsHot` 的打包策略与 bundle 命名
