...
| API | 用途 |
|-----|------|
| `AssetReference.Bind(IAssetHandle)` | 挂到实例；`OnDestroy` → `Release` 一次 |
| `BundleResLoader.LoadGameObject(loadPath)` | `Load` + `Instantiate` + 自动 `Bind` |
| `LoadGameObjectAsync` | 异步门面（依赖 B-2 inFlight 稳定后） |
...
| 非池自动 Release | `AssetsReference` | 模块 `OnDestroy` 手动 | `AssetReference` 组件 |
...
```text
[AssetRefTrace][Resource] key=Model/Prefabs/Bullet ref=1 delta=+1 reason=Load
[AssetRefTrace][Bundle] name=model_prefabs ref=1 delta=+1 reason=AcquireBundle
[AssetRefTrace][Pool] path=Model/Prefabs/Bullet share=2 delta=+1 reason=RegisterShare
[AssetRefTrace][Event] PrefabPool.TearDown path=Model/Prefabs/Bullet
```
...
| 低频 UI/NPC | `Load` + 字段保存 + `OnDestroy Release` | `LoadGameObject` + `AssetReference`（待实现） |
